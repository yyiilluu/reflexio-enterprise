-- Migration: Add agent_version filter to get_last_k_interactions
-- This enables filtering interactions by agent_version during feedback rerun

-- Drop existing function to update signature (adding p_agent_version parameter)
DROP FUNCTION IF EXISTS public.get_last_k_interactions(text, int, text[], bigint, bigint);

CREATE OR REPLACE FUNCTION public.get_last_k_interactions(
    p_user_id text DEFAULT NULL,  -- NULL means get all users
    p_limit int DEFAULT 100,
    p_sources text[] DEFAULT NULL,  -- Array of sources to filter by
    ...
    p_agent_version text DEFAULT NULL  -- Filter by agent_version on the request
)
RETURNS TABLE(
    request_id text,
