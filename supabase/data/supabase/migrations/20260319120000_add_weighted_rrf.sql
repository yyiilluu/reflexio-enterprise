-- Add weighted Reciprocal Rank Fusion (RRF) and upgrade FTS ranking to ts_rank_cd.
--
-- Changes to all 5 hybrid_match_* functions:
--   1. Add p_vector_weight and p_fts_weight parameters (default 1.0) for tunable fusion
--   2. Replace ts_rank() with ts_rank_cd(..., 1) for cover-density ranking with doc-length normalization
--   3. Apply weights in the hybrid RRF formula:
--        p_vector_weight * 1/(k + vec_rank) + p_fts_weight * 1/(k + fts_rank)
...
DROP FUNCTION IF EXISTS public.hybrid_match_profiles(
    public.vector, text, double precision, integer, bigint, text, text, integer, text
);
...
-- ========================================================
DROP FUNCTION IF EXISTS public.hybrid_match_interactions(
...
BEGIN
    ...
    fts_results AS (
        SELECT
            ...
            f.blocking_issue,
            f.status,
            ts_rank_cd(f.feedback_content_fts, tsquery_val, 1)::double precision as fts_score,
            ROW_NUMBER() OVER (ORDER BY ts_rank_cd(f.feedback_content_fts, tsquery_val, 1) DESC) as fts_rank
        FROM feedbacks f
        WHERE (p_search_mode = 'vector' OR f.feedback_content_fts @@ tsquery_val)
          AND f.status IS NULL
        ORDER BY ts_rank_cd(f.feedback_content_fts, tsquery_val, 1) DESC
        LIMIT CASE WHEN p_search_mode = 'vector' THEN 0 ELSE p_match_count * 3 END
    ),
    combined AS (
        SELECT
...
BEGIN
    ...
    fts_results AS (
        SELECT
            ...
            rf.status,
            rf.created_at,
            ts_rank_cd(rf.feedback_content_fts, tsquery_val, 1)::double precision as fts_score,
            ROW_NUMBER() OVER (ORDER BY ts_rank_cd(rf.feedback_content_fts, tsquery_val, 1) DESC) as fts_rank
        FROM raw_feedbacks rf
        WHERE (p_search_mode = 'vector' OR rf.feedback_content_fts @@ tsquery_val)
          AND rf.status IS NULL
          AND (p_filter_user_id IS NULL OR rf.user_id = p_filter_user_id)
        ORDER BY ts_rank_cd(rf.feedback_content_fts, tsquery_val, 1) DESC
        LIMIT CASE WHEN p_search_mode = 'vector' THEN 0 ELSE p_match_count * 3 END
    ),
    combined AS (
        SELECT
...
-- ========================================================
DROP FUNCTION IF EXISTS public.hybrid_match_skills(
    public.vector, text, double precision, integer, text, text, integer
);
