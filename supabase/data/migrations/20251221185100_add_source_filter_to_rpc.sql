-- Migration: Add source filtering to RPC functions
-- This enables filtering interactions by source when retrieving data

-- Add index on requests.source for efficient filtering
CREATE INDEX IF NOT EXISTS idx_requests_source ON requests(source);

-- Drop existing function to update signature (adding p_source parameter)
...
-- Drop existing function to update signature (adding p_source parameter)
DROP FUNCTION IF EXISTS public.get_new_request_interaction_groups(text, timestamp with time zone, bigint[]);
