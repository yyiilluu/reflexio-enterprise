...
class TestShouldRunAggregation:
    ...
    def test_rerun_flag_passed_to_count(self):
        ...
        assert call_kwargs.kwargs.get("min_raw_feedback_id") == 0 or call_kwargs[1].get("min_raw_feedback_id") == 0
...
class TestGetNewRawFeedbacksCount:
    def test_rerun_uses_zero_as_last_processed(self):
        ...
        assert result == 7
        assert agg.storage.count_raw_feedbacks.call_args.kwargs["min_raw_feedback_id"] == 0
    ...
    def test_non_rerun_reads_bookmark(self):
        ...
        with patch.object(
            FeedbackAggregator, "_create_state_manager"
        ) as mock_csm:
            mgr = MagicMock()
        ...
        assert result == 3
        assert agg.storage.count_raw_feedbacks.call_args.kwargs["min_raw_feedback_id"] == 42
...
class TestRun:
    ...
    def test_no_config_returns_early(self):
        ...
        req = FeedbackAggregatorRequest(
            agent_version="v1", feedback_name="fb"
        )
        agg.run(req)
    ...
    def test_run_fingerprint_state_updated(self, mock_gen, mock_clust):
        ...
        fingerprints_arg = call_kwargs.kwargs.get("fingerprints") or call_kwargs[1].get("fingerprints")
    ...
    @patch.object(FeedbackAggregator, "get_clusters")
    @patch.object(FeedbackAggregator, "_generate_feedback_from_clusters")
    def test_incremental_changed_clusters_but_no_archived_ids(self, mock_gen, mock_clust):
    ...
    @patch.object(FeedbackAggregator, "get_clusters")
    @patch.object(FeedbackAggregator, "_generate_feedback_from_clusters")
    def test_saved_fb_without_feedback_id_skipped_in_fingerprint_assignment(self, mock_gen, mock_clust):
        ...
        mgr.update_cluster_fingerprints.assert_called_once()
        call_kwargs = mgr.update_cluster_fingerprints.call_args
        new_fps = call_kwargs.kwargs.get("fingerprints") or call_kwargs[1].get("fingerprints")
    ...
    @patch.object(FeedbackAggregator, "get_clusters")
    @patch.object(FeedbackAggregator, "_generate_feedback_from_clusters")
    def test_exception_in_incremental_no_archived_ids_still_raises(self, mock_gen, mock_clust):
    ...
    @patch.object(FeedbackAggregator, "get_clusters")
    @patch.object(FeedbackAggregator, "_generate_feedback_from_clusters")
    def test_multiple_saved_feedbacks_assigned_to_multiple_fingerprints(self, mock_gen, mock_clust):
        ...
        assigned_ids = [v["feedback_id"] for v in new_fps.values() if v["feedback_id"] is not None]
    ...
    @patch.object(FeedbackAggregator, "get_clusters")
    @patch.object(FeedbackAggregator, "_generate_feedback_from_clusters")
    def test_incremental_carries_forward_unchanged_fingerprints(self, mock_gen, mock_clust):
