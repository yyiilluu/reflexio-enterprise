    ...
    def _process_results(self, results: list[list[UserProfile]]) -> None:
        ...
        if all_new_profiles:
            ...
            if is_deduplicator_enabled(self.org_id):
                ...
                # In rerun mode (output_pending_status), only dedup against PENDING profiles
                # to preserve existing CURRENT profiles
                existing_status_filter = (
                ...
                all_new_profiles, existing_ids_to_delete, superseded_profiles = (
                    deduplicator.deduplicate(
                        ...
                        user_id,
                        request_id,
                        existing_status_filter=existing_status_filter,
                    )
                )
                logger.info(
