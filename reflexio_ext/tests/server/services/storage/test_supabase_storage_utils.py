...
class TestParseIsoTimestamp:
    ...
    def test_variable_fractional_seconds_5_digits(self):
        ...
        result = _parse_iso_timestamp(ts)
        expected = int(datetime(2024, 3, 20, 8, 15, 30, 123450, tzinfo=UTC).timestamp())
        assert result == expected
    ...
    def test_variable_fractional_seconds_3_digits(self):
        ...
        result = _parse_iso_timestamp(ts)
        expected = int(datetime(2024, 3, 20, 8, 15, 30, 123000, tzinfo=UTC).timestamp())
        assert result == expected
    ...
    def test_fractional_seconds_6_digits(self):
        ...
        result = _parse_iso_timestamp(ts)
        expected = int(datetime(2024, 3, 20, 8, 15, 30, 123456, tzinfo=UTC).timestamp())
        assert result == expected
    ...
    def test_z_suffix_with_fractional(self):
        ...
        result = _parse_iso_timestamp(ts)
        expected = int(datetime(2024, 1, 1, 0, 0, 0, 500000, tzinfo=UTC).timestamp())
        assert result == expected
...
class TestSkillToData:
    ...
    def test_converts_skill_with_all_fields(self):
        ...
        skill = Skill(
            ...
            blocking_issues=[
                BlockingIssue(kind=BlockingIssueKind.MISSING_TOOL, details="no db tool")
            ],
...
class TestIsLocalhostUrl:
    ...
    def test_remote_host(self):
        ...
        assert (
            is_localhost_url("postgresql://user:pass@db.example.com:5432/db") is False
        )
...
class TestFeedbackAggregationChangeLogToData:
    ...
    def test_converts_change_log_with_all_entries(self):
        ...
        assert (
            data["updated_feedbacks"][0]["before"]["feedback_content"]
            == "before content"
        )
        assert (
            data["updated_feedbacks"][0]["after"]["feedback_content"] == "after content"
        )
...
class TestExecuteSqlFileDirect:
    ...
    def test_executes_select_statements(self, mock_psycopg2, mock_path_cls):
        ...
        results = execute_sql_file_direct(
            "postgresql://localhost/db", "/path/to/file.sql"
        )
