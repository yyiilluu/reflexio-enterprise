...
class TestInit:
    ...
    def test_standard_openai_init(self, mock_openai_cls):
        ...
        mock_openai_cls.assert_called_once_with(
            api_key="sk-test", timeout=config.timeout
        )
...
class TestGenerateResponse:
    ...
    def test_text_only_prompt(self):
        ...
        client.client.chat.completions.create.return_value = _make_text_response(
            "Paris"
        )
    ...
    def test_structured_output_pydantic_uses_parse_api(self):
        ...
        client = _build_client()
        client.client.chat.completions.parse.return_value = _make_parsed_response(
            parsed
        )
...
class TestEmbeddingRetryAndErrors:
    ...
    def test_embedding_all_retries_fail(self, mock_sleep):
        ...
        with pytest.raises(
            OpenAIClientError, match="embedding request failed after 2 attempts"
        ):
            client.get_embedding("text")

        assert (
            "API down" in str(client.client.embeddings.create.side_effect)
