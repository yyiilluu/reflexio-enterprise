...
from reflexio_ext.server.db.database import Base, engine
...
def generate_codes(count: int, expires_in_days: int | None = None) -> list[str]:
    ...
    if expires_in_days is not None:
        expires_at = int(datetime.now(UTC).timestamp()) + (expires_in_days * 86400)
...
def list_codes(show_used: bool = False) -> None:
    ...
    if client:
        ...
        from reflexio_ext.server.db.db_models import InvitationCode
    ...
    for row in rows:
        ...
        expires_str = (
            datetime.fromtimestamp(expires_at, tz=UTC).strftime("%Y-%m-%d %H:%M")
            if expires_at
            else "Never"
        )
